...
#include "..\..\llmozlib_dll\llmozlib_dll.h"
...
{
	public: 
	WorkingDirectorySetter(wchar_t* directory)
	{
                     ...
                     int       nCmdShow)
{
	...
	HMODULE module = 0;
	{
		WorkingDirectorySetter ws(L"mozilla");
		module = LoadLibrary(L"llmozlib_dll");
		if (! module) 
		{
			OutputDebugString(L"Failed loading llmozlib");
			return 1;
		}
	}
	...
	{
		OutputDebugString(L"Failed getting proc");
		return 1;
	}
	llmozlibInstance = get_instance_func();
	wchar_t buffer[200];
	wsprintf (buffer, L"LLMozLib version %S\n", llmozlibInstance->getVersion().c_str());
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_A1));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_A1);
