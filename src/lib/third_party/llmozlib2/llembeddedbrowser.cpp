...
bool LLEmbeddedBrowser::allowCursorInteraction ( bool allow )
{
	nsCOMPtr<nsIPref> pref = do_CreateInstance( NS_PREF_CONTRACTID );
	if ( pref )
	{
		pref->SetBoolPref( "bigworld.allowInteraction", allow );
		return true;
	}
	return false;
}
...
bool LLEmbeddedBrowser::destroyBrowserWindow( LLEmbeddedBrowserWindow* browserWindowIn )
{
	...
	//being done here to make sure the enableToolkitObserver is removed 
	browserWindowIn->enableToolkitObserver( false );
