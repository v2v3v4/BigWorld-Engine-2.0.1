	...
	public LLEmbeddedBrowserWindowObserver
{
	public:
		...
		void mouseButton( int button, int state, int xIn, int yIn )
		{
			...
			if ( button == GLUT_LEFT_BUTTON )
			{
				if ( state == GLUT_DOWN )
				{
					...
					LLMozLib::getInstance()->mouseDown( mBrowserWindowId, xIn, yIn, 0, 0 );
				}
				else
				if ( state == GLUT_UP )
				{
					// send event to LLMozLib
					LLMozLib::getInstance()->mouseUp( mBrowserWindowId, xIn, yIn, 0, 0 );
		...
		void mouseMove( int xIn , int yIn )
		{
			...
			// send event to LLMozLib
			LLMozLib::getInstance()->mouseMove( mBrowserWindowId, xIn, yIn, 0, 0 );
		...
		void* getNativeWindowHandle()
		{
			...
			#ifdef _WINDOWS
			return FindWindowA( NULL, mAppWindowName.c_str() );
			#else
