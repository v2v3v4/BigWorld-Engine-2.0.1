...
PRBool LLEmbeddedBrowserWindow::renderCaret()
{
	...
	if (! focusedContent->GetCurrentDoc())
		return NS_ERROR_FAILURE;
...
void LLEmbeddedBrowserWindow::mouseLeftDoubleClick( PRInt16 xPosIn, PRInt16 yPosIn, PRInt16 exactWidth, PRInt16 exactHeight )
{
	...
	sendMozillaMouseEvent( NS_MOUSE_LEFT_BUTTON_DOWN, xPosIn, yPosIn, clickCount, exactWidth, exactHeight );
}

////////////////////////////////////////////////////////////////////////////////
// higher level mouse event
void LLEmbeddedBrowserWindow::mouseDown( PRInt16 xPosIn, PRInt16 yPosIn, PRInt16 exactWidth, PRInt16 exactHeight )
{
...
////////////////////////////////////////////////////////////////////////////////
// higher level mouse event
void LLEmbeddedBrowserWindow::mouseUp( PRInt16 xPosIn, PRInt16 yPosIn, PRInt16 exactWidth, PRInt16 exactHeight )
{
	const PRUint32 clickCount = 1;
	sendMozillaMouseEvent( NS_MOUSE_LEFT_BUTTON_UP, xPosIn, yPosIn, clickCount, exactWidth, exactHeight );
}

////////////////////////////////////////////////////////////////////////////////
// higher level mouse event
void LLEmbeddedBrowserWindow::mouseMove( PRInt16 xPosIn, PRInt16 yPosIn, PRInt16 exactWidth, PRInt16 exactHeight )
{
	const PRUint32 clickCount = 1;	// ignored?
	sendMozillaMouseEvent( NS_MOUSE_MOVE, xPosIn, yPosIn, clickCount, exactWidth, exactHeight );
}
...
//get screen bounds (Translated based on the root component as we are mapping meaning root is 0,0 and all the rest 
//are in root widget pixel coordinates
void getTranslatedScreenBounds(nsCOMPtr< nsIWidget > currWidget, nsCOMPtr< nsIWidget > rootWidget, nsRect& aRect)
{
	nsRect currWidgetRect;
	currWidget->GetScreenBounds(currWidgetRect);
	nsRect rootWidgetRect;
	rootWidget->GetScreenBounds(rootWidgetRect);
	aRect.x = currWidgetRect.x - rootWidgetRect.x;
	aRect.y = currWidgetRect.y - rootWidgetRect.y;
	aRect.width = currWidgetRect.width;
	aRect.height = currWidgetRect.height;
}
...
PRBool LLEmbeddedBrowserWindow::sendMozillaMouseEvent( PRInt16 eventIn, PRInt16 xPosIn, PRInt16 yPosIn, PRUint32 clickCountIn, PRInt16 exactWidth, PRInt16 exactHeight )
{
	if ( ! mEnabled )
		return PR_FALSE;
	...
	nsCOMPtr< nsIWidget > rootWidget = rootView->GetWidget();
	if ( ! rootWidget )
		return PR_FALSE;


	nsMouseEvent mouseEvent( PR_TRUE, eventIn, rootWidget, nsMouseEvent::eReal );
	...
	{
		isMeta = (0x2aa << 22) | (exactHeight << 11) | exactWidth;
	}
	mouseEvent.isMeta = isMeta;
	mouseEvent.widget = rootWidget;
...
{
	//for better tab handling we focus the browser when sent.
	sendMozillaKeyboardEvent( 0, keyCode, 0 );
}

// accept keyboard input that's already been translated into a unicode char.
void LLEmbeddedBrowserWindow::unicodeInput( PRUint32 uni_char, PRUint32 codePage )
{
	sendMozillaKeyboardEvent( uni_char, 0, codePage );
}

////////////////////////////////////////////////////////////////////////////////
// synthesizes a keyboard event and sends into the embedded instance
PRBool LLEmbeddedBrowserWindow::sendMozillaKeyboardEvent( PRUint32 uni_char, PRUint32 ns_vk_code, PRUint32 codePage)
{
	//send the normal keyboard event
	if ( ! mEnabled )
		return PR_FALSE;
	...
	nsKeyEvent keyEvent( PR_TRUE, NS_KEY_PRESS, rootWidget );
	...
	keyEvent.isAlt = 0;
	keyEvent.isMeta = 0;
	keyEvent.widget = rootWidget;
	keyEvent.nativeMsg = nsnull;
	keyEvent.point.x = lastMouseX_;
	keyEvent.point.y = lastMouseY_;
	keyEvent.refPoint.x = lastMouseX_;
	keyEvent.refPoint.y = lastMouseY_;
	keyEvent.flags = 0;
	...
	//hack for mozilla flash pages send a special mouse move containing the key event data.
	if ( ! mEnabled )
		return PR_FALSE;
	...
	viewManager = presContext->GetViewManager();
	if ( ! viewManager )
		return PR_FALSE;
	...
	nsMouseEvent mouseEvent( PR_TRUE, NS_MOUSE_MOVE, widget, nsMouseEvent::eReal );
	mouseEvent.clickCount = 1;
	mouseEvent.isShift = 0;
	mouseEvent.isControl = 0;
	mouseEvent.isAlt = /*code page is given in the alt*/codePage;
	mouseEvent.isMeta = /*flag to mark nativemsg contains the real event*/(PRBool)0xB16802D;
	mouseEvent.widget = widget;
	mouseEvent.nativeMsg = &keyEvent;
	mouseEvent.point.x = lastMouseX_;
	mouseEvent.point.y = lastMouseY_;
	mouseEvent.refPoint.x = lastMouseX_;
	mouseEvent.refPoint.y = lastMouseY_;
	mouseEvent.flags = 0;
...
void LLEmbeddedBrowserWindow::cleanMemory()
{
	delete[] mPageBuffer;
	mPageBuffer = 0;
}
