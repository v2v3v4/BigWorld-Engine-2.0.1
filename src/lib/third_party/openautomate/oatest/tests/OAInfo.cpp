...
oaChar ExePath[MAX_PATH];
oaChar ExeName[MAX_PATH];

bool ParseFile(const char* exe_path, 
                                         oaString file_path, 
                                         oaString file_name)
{
#if WIN32
  ...
  int PathLength;
  for ( PathLength = dwRet; PathLength > 0; PathLength-- )
  {
    if ( exe_path[PathLength-1] == delim )
      break;
  }
  ...
  {
    file_path[i] = (oaChar)exe_path[i]; 
  }
  file_path[i] = '\0';

  int j = 0;
  for ( i = PathLength; i < dwRet; i++)
  {
    file_name[j++] = (oaChar)exe_path[i];
  }
  file_name[j] = '\0';
  return true;
}

OAC_SIMPLE_TEST_HEAD(OAInfo, 
                     "Basic sanity tests for OA Versioning",
                     2, 
                     true) 

OAC_TEST_MSG(Context()->GetCurrentServer() != NULL,
              "No Server was created.");

OAC_MSG("OA Versions: [Major.Minor.Custom.Build]")

OAC_MSG("RPC version: "
        << Context()->GetCurrentServer()->Versions.ClientRPCVersionMajor <<"."
        << Context()->GetCurrentServer()->Versions.ClientRPCVersionMinor <<"" )

OAC_MSG("Application OA version: "
        << Context()->GetCurrentServer()->Versions.AppOAVersionMajor <<"."
        << Context()->GetCurrentServer()->Versions.AppOAVersionMinor <<"."
        << Context()->GetCurrentServer()->Versions.AppOABuild <<"."
        << Context()->GetCurrentServer()->Versions.AppOACustom <<"" )

OAC_MSG("Plug-in OA version: "
        << Context()->GetCurrentServer()->Versions.ClientOAVersionMajor <<"."
        << Context()->GetCurrentServer()->Versions.ClientOAVersionMinor <<"."
        << Context()->GetCurrentServer()->Versions.ClientOABuild <<"."
        << Context()->GetCurrentServer()->Versions.ClientOACustom <<"" )

OAC_COND_WARN(Context()->GetCurrentServer()->Versions.ClientOAVersionMajor <=
              Context()->GetCurrentServer()->Versions.AppOAVersionMajor,
              "The Application was built with a version of OA that may not "
              "be compatible with the plugin. Most likely, the application "
              "needs to be built with an updated version of OpenAutomate.");
