...
#define TEST(cond) test_condition(__FILE__, __LINE__, #cond, cond);

#define TEST_TEMP_DIR test_TempDir
#define TEST_DATE_DIR test_DataDir

#ifdef TEST_SLAVE_FILE
extern void test_condition(
                           ...
                           bool        condition);
#else
  static int test_NTestsRan = 0;
  static int test_NTestsSucceeded = 0;
  static int test_NTestsPlanned = 0;
  static char test_TempDir[1024] = "./tmp";
  static char test_DataDir[1024] = "./test_data";
  static std::ostrstream oStrStream;

static void test_OverrideEnvStr(const char *env_var_name,
                                char *str)
{
  const char *EnvVal = getenv(env_var_name);
  if(EnvVal)
    strcpy(str, EnvVal);
}

static void test_DumpTestResults(int argc, char *argv[])
{
  bool test_Sucess = test_NTestsPlanned == test_NTestsSucceeded;
  if(test_Sucess)
  {
    fprintf(stderr, 
            "- %d tests ran successfully (%d planned)!!!\n",
            test_NTestsRan,
            test_NTestsPlanned); 
  }
  else
  {
    fprintf(stderr, 
            "- %d out of %d tests failed. (%d planned)\n",
            test_NTestsRan - test_NTestsSucceeded, 
            test_NTestsRan,
            test_NTestsPlanned); 
  }
  const char *test_HarnessRun = getenv("NVM_HARNESS_RUN");
  if(test_HarnessRun && atoi(test_HarnessRun))
  {
    oStrStream << std::ends;
    std::cout << "# @@ NVM HARNESS XML v. 1.0" << std::endl 
      << "<TestingScript status='" << (test_Sucess? "OK" : "FAIL")
      << "' full_path='" <<  argv[0] << "' tests_planned='" 
      << test_NTestsPlanned << "'>" << std::endl
      << "\t<Tests>"<< std::endl 
      << oStrStream.str() 
      << "\t</Tests>" << std::endl
      << "</TestingScript>"; 
...
void test_condition(const char* file_name, 
                    int         line_num, 
                    const char* condition_string, 
                    bool        condition)
{
  ...
  if(condition)
  {
    oStrStream  << "\t\t<Test line_no='" 
                << line_num 
                << "' id='" 
                << test_NTestsRan << "' type='KNOWN'>"; 

    oStrStream << std::endl << "\t\t\t<Status>OK</Status>";

    test_NTestsSucceeded++;
  }
  else
  {
    oStrStream  << "\t\t<Test line_no='" 
    ...
    std::cerr   << "FAILURE: Test condition (" 
                << condition_string
                << ") failed at "
                << file_name
                << ":" 
    ...
    oStrStream  << std::endl << "\t\t\t<Status>FAIL</Status>" 
                << std::endl << "\t\t\t<Error>" 
                << std::endl << "\t\t\t\t<Got>&apos;&apos;</Got>"
                << std::endl << "\t\t\t\t<Context"
                << " line_no='" << line_num 
                << "' end_line='" << line_num 
                << "' begin_line='" << line_num << "'>"
                << std::endl << "\t\t\t\t\t" << condition_string
                << std::endl << "\t\t\t\t</Context>"
                << std::endl << "\t\t\t</Error>";
  }

  oStrStream << std::endl << "\t\t</Test>" << std::endl;
}

static void test_CommonInit(int argc, char *argv[])
{
  test_OverrideEnvStr("TEST_DATA_DIR", test_DataDir);
  test_OverrideEnvStr("TEST_TEMP_DIR", test_TempDir); 
}

#define TEST_INIT  \
  int main(int argc, char *argv[]) \
  { \
    test_CommonInit(argc, argv);

#define TEST_INIT_PLANNED(tests_planned) \
  TEST_INIT \
    test_NTestsPlanned = tests_planned;

#define TEST_SET_PLANNED(tests_planned) \
    test_NTestsPlanned = tests_planned;

#define TEST_SET_PLANNED_DEFAULT \
  test_NTestsPlanned = test_NTestsRan;

#define TEST_END \
  test_DumpTestResults(argc, argv);\
  exit((test_NTestsSucceeded - test_NTestsPlanned) ? -1 : 0);\
}
...
/* Starts a process asynchronously, and returns a handle to it.  Returns NULL
   on error. */
static void *test_RunCmdAsync(const char *cmd)
{
  BOOL Ret;
  test_Process *Process = (test_Process *)malloc(sizeof(test_Process));
  char *TmpCmd = strdup(cmd);

  assert(cmd != NULL);

  GetStartupInfoA(&Process->ChildStartupInfo);

  Process->ChildStartupInfo.dwFlags = STARTF_USESTDHANDLES;
...
/* Kills a running process.  Returns the exit code */
static int test_KillProcess(void *process)
{
  assert(process != NULL);

  assert("NOT DEFINED YET FOR THIS PLATFORM!" == NULL);

  return(0);
}

/* Waits for a process to exit.  */
static int test_WaitProcess(void *process, int *exit_code)
{
  DWORD Ret;
  DWORD ExitCode;
  ...
  Ret = WaitForSingleObject(Process->ProcInfo.hProcess, INFINITE);

  if(Ret == WAIT_FAILED)
    return(0);

  if(GetExitCodeProcess(Process->ProcInfo.hProcess, &ExitCode))
  {
    *exit_code = ExitCode;
    return(1);
  }
...
/* Waits for a process to exit.  */
static int test_WaitProcess(void *process)
{
  assert("NOT DEFINED YET FOR THIS PLATFORM!" == NULL);
}
