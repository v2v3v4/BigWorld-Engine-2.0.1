...
TEST_INIT_PLANNED(10)
  ...
  oaRPCSerializeString(Buf, TestStr);
  oaRPCSerializeInt(Buf, 69);
  oaRPCSerializeFloat(Buf, 3.1419);
  oaRPCSerializeBool(Buf, OA_TRUE);
  oaRPCSerializeString(Buf, "");
  oaRPCSerializeBool(Buf, OA_FALSE);


  oaRPCSize Offset = 0;
  const oaChar *Str = oaRPCDeserializeString(Buf, &Offset);
  TEST(Str != NULL);
  TEST(!strcmp(Str, TestStr));

  TEST(oaRPCDeserializeInt(Buf, &Offset) == 69);
  TEST(oaRPCDeserializeFloat(Buf, &Offset) == 3.1419);
  TEST(oaRPCDeserializeBool(Buf, &Offset) == OA_TRUE);
  TEST(*oaRPCDeserializeString(Buf, &Offset) == '\0');
  TEST(oaRPCDeserializeBool(Buf, &Offset) == OA_FALSE);
