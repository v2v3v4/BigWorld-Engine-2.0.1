...
/*
static oauAppInfo TestData;

void init_data()
{
  TestData.DevName = TEXT("Test Developer");
  TestData.AppName = TEXT("Test Application");
  TestData.AppBuildName = TEXT("TestBuild_0331");

  TestData.AppValue.RootPath = TEXT("C:\\Test\\testbuild");
  TestData.AppValue.Entryexe = TEXT("test_app.exe");
  TestData.AppValue.InstallDate = TEXT("2008-03-31T13:23:01");
  TestData.AppValue.Region = TEXT("APAC");
}
*/
TEST_INIT_PLANNED(0)
...
do
{
  ...
  //TEST(oauRegisterApplication(TestData) == OA_TRUE);
  //TEST(oauUnregisterApplication(TestData) == OA_TRUE);
  //oaInt oauGetRegisteredApplications(oauApplicationInfo **info);

  //TEST(oauRegisterApplicationXML(TestData) == OA_TRUE);
