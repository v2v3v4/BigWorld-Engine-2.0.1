...
bool RegisterApp(oaChar* app_name, 
                 oaChar* build_version,
                 oaChar* developer_name, 
                 oaChar* executable,
                 oaChar* app_root_dir, 
                 oaChar* region )
{

  oauAppBuildId AppIds;

  AppIds.AppName = app_name;  
  AppIds.AppBuildName = build_version;
  AppIds.DevName = developer_name; 

  oauAppBuildInfo AppInfo;
 
  /*
   * If ther other cmdline args that are needed to run
   * in a fully automated instance add them here.
   */
  AppInfo.EntryExe = executable;

  /* Root path for the application */
  AppInfo.InstallRootPath = app_root_dir;

  /* Regions supported by build in RFC3066 format. */
  AppInfo.Region =  region;

  SYSTEMTIME  tm;
  GetLocalTime(&tm);
  AppInfo.InstallDateTime = (oaChar *)malloc(sizeof(oaChar)*20);
  sprintf( AppInfo.InstallDateTime, "%04d-%02d-%02d %02d:%02d:%02d",
    tm.wYear,
    tm.wMonth,
    tm.wDay,
    tm.wHour,
  ...
  if(oauRegisterAppBuild(OAU_REGISTER_FILE_ENV, &AppIds, &AppInfo))
    return true;

  if(oauRegisterAppBuild(OAU_REGISTER_FILE_HOME, &AppIds, &AppInfo))
    return true;

  if(oauRegisterAppBuild(OAU_REGISTER_FILE_SYSTEM, &AppIds, &AppInfo))
    return true;

  if(oauRegisterAppBuild(OAU_REGISTER_REGISTRY_USER, &AppIds, &AppInfo))
    return true;

  if(oauRegisterAppBuild(OAU_REGISTER_REGISTRY_SYSTEM, &AppIds, &AppInfo))
    return true;
...
int main(int argc, char *argv[])
{
  ...
  if (!RegisterApp("Simple App",
                   "1.0",
                   "NVIDIA Corp",
                   "bin\\simple_app.exe -slientmode",
                   "c:\\SomePath",
                   "en_US|ko_KR" ))
  {
    Error("App didn't register.\n");
  }
