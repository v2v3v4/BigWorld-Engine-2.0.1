...
#include "cstdmf/string_utils.hpp" // BigWorld
...
#define _CHS_HKL_GOOGLE ( (HKL)0xE0200804 ) // Google Pinyin (BigWorld)
#define _CHS_HKL_SOGOU  ( (HKL)0xE0210804 ) // Sogou (BigWorld)
#define _CHS_HKL_SOGOU_WUBI ( (HKL)0xE0220804 ) // Sogou Wubi (BigWorld)
		        ...
		        if ( lParam & GCS_COMPSTR )
		        {
					...
					if ( GETLANG() == LANG_CHT && !GetImeId() )
					{
						...
						if (g_szCompositionString[0] != '\0')
						{
							...
							StringCchCopy(g_szReadingString, COUNTOF(g_szReadingString), szCompStr);
							ZeroMemory( g_szCompositionString, COUNTOF(g_szCompositionString) );
