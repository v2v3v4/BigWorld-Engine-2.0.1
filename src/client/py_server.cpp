...
PyObject * ServerCaller::pyCall( PyObject * args )
{
	...
	if (pServer != NULL && entity_.id() < (1L<<30) && pServer->online())
	{
		...
		EntityID playerID = EntityManager::instance().proxyEntityID();
		...
		if (isProxyCaller_)
		{
			if (playerID == entity_.id())
			{
				pStream = &pServer->startProxyMessage( msgID );\
			}
			else
			{
		...
		else
		{
			if (playerID == entity_.id())
			{
				pStream = &pServer->startAvatarMessage( msgID );
			}
			else
			{
				pStream = &pServer->startEntityMessage( msgID, entity_.id() );
			}
		}
...
PyObject * PyServer::pyAdditionalMembers( PyObject * pBaseSeq )
{
	...
	MF_ASSERT_DEV( index == PyTuple_GET_SIZE( pTuple ) );
